            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    if row['범주'] in categoryName:
                        ...
                        categoryName.append(str(row['범주']).strip().replace('"', '\\"').replace("'", "\\'"))
                        categoryIndex.append(categoryNum)
                        categoryInclude.append(1)
                        categoryNum += 1

                    hint += 'EPD(Db("' + str(row["힌트1"]).strip().replace('"', '\\"').replace("'", "\\'") + '")), '
                    artist += 'EPD(Db("' + str(row["가수"]).strip().replace('"', '\\"').replace("'", "\\'") + ' - ' + str(row["제목"]).strip().replace('"', '\\"').replace("'", "\\'") + '")), '

                    count = 0
                    answers = str(row['정답 리스트1']).replace('"', '\\"').replace("'", "\\'").split(',')
                    count += len(answers)

                    if pd.notna(row['정답 리스트2']):
                        answers += str(row['정답 리스트2']).replace('"', '\\"').replace("'", "\\'").split(',')
